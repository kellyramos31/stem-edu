...
export default function CommentsOnPost(props) {
  ...
  const { deleteComment } = useContext(PostCommentContext);
    ...
    <div>
      <h3 className="public-comments">
        ...
        {_comments.map((_comment, index) => (
          ...
          >
            <span className="user-name-span-comment">
              {_comment._user.username}
            </span>{" "}
            ...
            {username === _comment._user.username ? (
              <div
                key={index}
                id={_comment._id}
                className="edit-del-comment-btns"
              >
                <div className="edit-del-btns-group">
                  <button
                    className="delete-comment-btn"
                    onClick={() => deleteComment(_comment._id, _comment._post)}
                  >
                    <RiDeleteBin6Fill size={15} style={{ fill: "royalblue" }} />
                  </button>
                  <button
                    className="edit-comment-btn"
                    onClick={() =>
                      toggleToEdit(index, _comment._id, _comment._post)
                    }
                  >
                    <FaEdit size={15} style={{ fill: "royalblue" }} />
                  </button>
                </div>
              </div>
            ) : (
