...
export default function PostCommentProvider(props) {
  ...
  function addPost(newUserPost) {
    userAxios
      .post("/api/forumpost", newUserPost)
      .then((res) => {
        console.log(res);
        setPostState((prevState) => ({
          ...prevState,
  ...
  function deletePost(postId) {
    ...
    userAxios
      ...
      .then((res) => {
        setPostState((prevState) => ({
          userPosts: prevState.userPosts.filter(
            (userPost) => userPost._id !== postId
          ),
        }));
        getPosts();
      })
  ...
  function editPost(inputs, postId) {
    ...
    userAxios
      .put(`/api/forumpost/${postId}`, inputs)
      .then((res) => {
        setPostState((prevState) =>
          prevState.userPosts.map((userPost) =>
            userPost._id !== postId ? userPost : res.data
          )
        );
      })
      .catch((err) => console.log(err.response.data.errMsg));
  ...
  function addCommentTally(postId) {
    ...
    userAxios
      .put(`/api/forumpost/increment/${postId}`, postId)
      .then((res) => {
        console.log("addComment res", res);
        setPostState((prevState) => ({
          ...prevState,
  ...
  function deleteCommentFromPostArray(commentId, postId) {
    ...
    userAxios
      .put(`/api/comment/deleteCommentFromPost/${commentId}`, _post)
      // console.log("commentId:", commentId)
      .then((res) => {
  ...
  function minusCommentTally(postId) {
    ...
    userAxios
      .put(`/api/forumpost/decrement/${postId}`, postId)
      .then((res) => {
  ...
  function editComment(inputs, commentId) {
    ...
    console.log("inputs for edit", inputs);
    userAxios
      .put(`/api/comment/${commentId}`, inputs)
      .then((res) => {
        setPostState((prevState) =>
          prevState.posts.map((post) =>
            post._comment !== commentId ? post.userComment : res.data
          )
        );
      })
      .catch((err) => console.log(err.response.data.errMsg));
  ...
  function handleMenuPosts(e) {
    ...
    userAxios
      .get(`/api/forumpost/search/category?category=${e.target.value}`)
      .then((res) => {
  ...
  function handlePostSearch(e) {
    ...
    userAxios
      .get(`/api/forumpost/search/userterm?searchTerm=${e.target.value}`)
      .then((res) => {
        setPostState({ posts: res.data });
