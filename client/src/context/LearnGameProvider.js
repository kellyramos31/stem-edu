...
export default function LearnGameProvider(props) {
  ...
  function getGameQuestions() {
    ...
    userAxios
      .get("/api/learngame/play")
      .then((res) => {
        console.log("res from learnGameProvider:", res);
        setGameState((prevState) => ({
          ...prevState,
          questions: res.data.sort(() => Math.random() - 0.5), //note: would be better to use Fisher-Yates algorithm, but this does move them around
        }));
      })
  ...
  function saveMyScore(gameScore) {
    ...
    userAxios
      .post("/api/learngame/play/score", scoreTotal)
      .then((res) => {
        console.log(res);
  ...
  function getMyGameScores() {
    userAxios
      .get("/api/learngame/play/score/user")
      .then((res) => {
        console.log(res);
  ...
  function incrementUserBadgeCount() {
    ...
    userAxios
      .put("/api/learngame/play/badge/increment")
      .then((res) => {
  ...
  function getBadgeCount() {
    userAxios
      .get(`/api/learngame/play/badge/count`)
      .then((res) => {
        console.log(res);
  ...
  function handleMenuFilter(e) {
    ...
    userAxios
      .get(
        `/api/learngame/learn/search/categorySTEM?categorySTEM=${e.target.value}`
      )
      .then((res) => {
  ...
  function filterAnsweredQuestion(_id) {
    setGameState((prevState) => ({
      questions: prevState.questions.filter((question) => question._id !== _id),
    }));
  }
  ...
  function handleFlashcardSearch(e) {
    ...
    userAxios
      .get(`/api/learngame/learn/search?searchTerm=${e.target.value}`)
      .then((res) => {
        setFlashcardState({ flashcards: res.data });
  ...
  function addFlashcard(newFlashcard) {
    userAxios
      .post("/api/learngame/learn", newFlashcard)
      .then((res) => {
        console.log(res);
        setFlashcardState((prevState) => ({
          ...prevState,
